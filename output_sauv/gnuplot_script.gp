...
set ylabel 'x(t)'
set style fill noborder
plot 'x2outer.out' using 1:2 w l lt 3 lw 2 lc rgb '#4dbeee'  title "maximal outer flowpipe", 'x2outer.out' using 1:3 w l lt 3 lw 2 lc rgb '#4dbeee'  notitle, 'x2inner.out' using 1:2:3 w filledcu lc rgb '#4dbeee'  title "maximal inner flowpipe", 'x2outer_minimal.out' using 1:2 w l lt 7 lc rgb '#7e2f8e' lw 2 title "minimal outer flowpipe", 'x2outer_minimal.out' using 1:3 w l lt 7 lc rgb '#7e2f8e' lw 2 notitle,'x2inner_minimal.out' using 1:2:3 w filledcu lc rgb '#7e2f8e' title "minimal inner flowpipe" 
unset output
set term pngcairo font "Helvetica,18"
set output "xi.png"
set xlabel 't (seconds)'
